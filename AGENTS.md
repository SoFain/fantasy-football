...
- A source-verified Sleeper depth-chart false positive may be classified as review-only only through an exact, bounded entry in `src/ranking_owner_decisions.py`. The exemption must expire automatically when team, experience, games, or qualification volume leaves the recorded bounds.

-- Review-only WR bridge from source-local situational identities to official warehouse IDs.
-- Mirrors situational_identity_bridge_review with the WR position pool.
CREATE OR REPLACE VIEW `{{PROJECT_ID}}.{{DATASET_ID}}.v_wr_fable_v1_identity_bridge` AS
WITH source_candidates AS (
  ...
  WHERE source_position = 'WR'
),
identity_overrides AS (
  -- Source-local override for the current ARI receiver. The shared name bridge also
  -- contains his father, so name normalization alone must continue to fail closed.
  SELECT
    'WR' AS source_position,
    'd96487d' AS situational_player_id,
    'marvin-harrison' AS player_slug,
    '00-0039849' AS candidate_internal_player_id,
    'MARVIN_HARRISON_JR_SOURCE_ID' AS identity_override_reason
),
bridge_names AS (
  SELECT
...
resolved AS (
  SELECT
    ...
    override.candidate_internal_player_id IS NOT NULL AS identity_override_applied,
    override.identity_override_reason,
    ...
    CASE
      WHEN override.candidate_internal_player_id IS NOT NULL
        THEN override.candidate_internal_player_id
    ...
    CASE
      WHEN override.candidate_internal_player_id IS NOT NULL THEN 'VERIFIED'
    ...
    CASE
      WHEN override.candidate_internal_player_id IS NOT NULL THEN 1.0
  ...
  FROM source_candidates AS source
  LEFT JOIN identity_overrides AS override
    USING (source_position, situational_player_id, player_slug)
...
SELECT
  *,
  ...
  NOT identity_override_applied
    AND (
      exact_match_count > 1
      OR (exact_match_count = 0 AND name_team_season_match_count > 1)
    ) AS duplicate_identity_collision
FROM resolved;

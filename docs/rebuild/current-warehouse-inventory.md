...
| `depth_charts` | raw/source | `src/player_profiles.py` | `src/pipeline.py:run_pipeline` | one player-team-date depth chart row, inferred | range partition by `season`, `src/load.py:load_df_to_partitioned_table`; filtered in `src/transform.py:transform_depth_charts_data` | Not safe directly. Use current role mart. |
...
| `active_league_rosters` | deprecated/unknown | prompt exposure only | no writer found | unknown | unknown | Not safe. Remove from prompt or replace with Sleeper marts. |
| `historical_player_metrics` | deprecated/unknown alias | prompt text says alias for `weekly_metrics` | no writer found | unknown alias | unknown | Not safe. Replace with `analytics_player_weekly_truth` or future feature mart. |

...
| UI-004 | `render_value_analyzer.query_player_history`, `app.py:3147-3155` | `weekly_metrics` | Provides recent stats for AI trade outlook. | Streamlit now has `USE_COMPAT_TRADE_PLAYER_HISTORY=false` wiring to `src/trade_history.py`. Default behavior is unchanged. If the flag is enabled and compat data fails, the view shows a warning before falling back to legacy. | `compat_trade_player_history`, feature-flagged default off. |
...
8. P1: Validate and gradually enable Trade Lab `compat_trade_assets_current` and `compat_trade_player_history` wiring.

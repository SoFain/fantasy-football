...
This file classifies every BigQuery table referenced by `src/`. It is documentation only.

The "safe to expose" column now describes exposure to any future consumer surface. The Streamlit UI and its chat surface were retired, so no table is currently exposed to a prompt.
...
| --- | --- | --- | --- |
| `INFORMATION_SCHEMA.TABLES` | admin metadata | Used by `src/materialize.py:get_existing_tables`, to inspect table existence. | Admin only. |
| `dashboard_job_runs` | admin metadata | Written by the retired Streamlit admin console. Superseded by `cloud_run_job_runs`, written by `src/cloud_run_jobs.py`. | Admin UI only. |
...
| `play_by_play` | raw/source | Loaded from nflreadpy by `src/pipeline.py:run_pipeline`; read by `src/materialize.py:build_game_environment_sql`, `src/materialize.py:build_player_weekly_truth_sql`, and `src/materialize.py:build_player_qb_weekly_sql`. | Not safe directly. |
| `weekly_metrics` | raw/source | Loaded by `src/pipeline.py:run_pipeline`; read by UI at `src/sleeper_watch.py` and `src/trade_history.py`; read by mart builder at `src/materialize.py:build_player_weekly_truth_sql`. | Not safe directly. |
| `team_descriptions` | raw/source | Loaded by `src/pipeline.py:run_pipeline`; replicated by `src/transform.py:transform_team_data`. | Not safe directly. |
| `draft_picks` | raw/source | Loaded by `src/pipeline.py:run_pipeline`; extracted in `src/extract.py:get_draft_picks_data`; filtered by `src/transform.py:transform_draft_picks_data`. | Not currently UI-facing. |
| `player_rosters` | raw/source | Loaded by `src/pipeline.py:run_pipeline`; read directly in Player Profiles at `src/player_profiles.py`; read in marts at `src/materialize.py:build_player_weekly_truth_sql`, `src/materialize.py:build_pigskin_rankings_sql`, and `src/materialize.py:build_pigskin_rankings_sql`. | Not safe directly. |
| `player_contracts` | raw/source | Loaded by `src/pipeline.py:run_pipeline`; read directly by Player Profiles at `src/player_profiles.py`. | Not safe directly. |
| `ngs_passing` | raw/source | Loaded by `src/pipeline.py:run_pipeline`. | Not safe directly. |
| `ngs_rushing` | raw/source | Loaded by `src/pipeline.py:run_pipeline`. | Not safe directly. |
| `ngs_receiving` | raw/source | Loaded by `src/pipeline.py:run_pipeline`; read by `src/materialize.py:build_player_weekly_truth_sql`. | Not safe directly. |
...
| `sleeper_league_users` | raw/source | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`. | Not safe directly. |
| `sleeper_rosters` | raw/source | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`; read directly at `src/sleeper_watch.py`. | Not safe directly. |
| `sleeper_roster_players` | raw/source | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`; read directly at `src/sleeper_watch.py`, `src/viewer_team_context.py`, and `src/viewer_team_context.py`. | Not safe directly. |
| `sleeper_matchups` | raw/source | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`. | Not safe directly. |
| `sleeper_lineups` | raw/source | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`; read directly at `src/viewer_team_context.py`. | Not safe directly. |
| `sleeper_available_players` | raw/source | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`; read directly at `src/viewer_team_context.py`. | Not safe directly. |
| `sleeper_viewer_team_snapshots` | staging | Schema in `src/ingest_sleeper_league.py:SLEEPER_TABLE_SCHEMAS`; rows assembled at `src/ingest_sleeper_league.py:build_records`; read directly at `src/viewer_team_context.py`. | Not safe directly. |
| `market_values` | staging | FantasyCalc table recreated by `src/fetch_market_values.py:upload_to_bigquery`; legacy direct UI debt remains in `src/pigskin_context_tools.py`; controlled mart read exists in `src/materialize_trade_assets.py`. | Not safe directly. |
| `college_player_stats` | raw/source | Schema in `src/setup_college_tables.py:create_college_tables`; loaded by `src/ingest_college_data.py:upload_to_bigquery`; read directly at `src/player_profiles.py`. | Not safe directly. |
| `rookie_scouting_metrics` | staging | Schema in `src/setup_college_tables.py:create_college_tables`; loaded from UI CSV by the "Upload and Import Scouting Metrics" control in `app.py`; read directly at `src/player_profiles.py`. | Not safe directly. |
| `analytics_context_events` | feature-like mart | Curated context loaded by `src/ingest_context_events.py:load_context_events`; prompt guidance at `src/pigskin_context_tools.py`. | Safe if source status is labeled. |
| `analytics_external_context_search_results` | feature-like mart | Created and loaded by `src/verify_player_context.py:ensure_results_table` and `src/verify_player_context.py:store_results`; read by the retired Trade Lab UI; no current consumer. | Safe as stored search leads, not final truth. |
| `analytics_api_usage_daily` | admin metadata | Created/read/merged by `src/verify_player_context.py:ensure_usage_table`, `src/verify_player_context.py:get_request_count`, and `src/verify_player_context.py:increment_request_count`. | Admin only. |
| `analytics_game_environment` | feature-like mart | Built from `play_by_play` by `src/materialize.py:build_game_environment_sql`; prompt guidance at `src/pigskin_context_tools.py`. | Safe. |
| `analytics_player_qb_weekly` | feature-like mart | Built from `play_by_play` by `src/materialize.py:build_player_qb_weekly_sql`; read by `analytics_player_weekly_truth` at `src/materialize.py:build_player_weekly_truth_sql`. | Safe with sample caveat. |
| `analytics_player_qb_splits` | feature-like mart | Built from `analytics_player_qb_weekly` by `src/materialize.py:build_player_qb_splits_sql`; prompt guidance at `src/pigskin_context_tools.py`. | Safe with sample caveat. |
| `analytics_player_weekly_truth` | feature-like mart | Built by `src/materialize.py:build_player_weekly_truth_sql`; read throughout UI and mart code, including `src/sleeper_watch.py`, `src/player_profiles.py`, `src/viewer_team_context.py`, and `src/materialize.py:build_fraud_watch_sql`. | Safe current default table. |
| `analytics_fraud_watch` | output | Built by `src/materialize.py:build_fraud_watch_sql`; read by `src/segment_packets.py`, and `src/segment_packets.py`. | Safe show output. |
| `analytics_pigskin_rankings_candidates` | feature-like mart | Built by `src/materialize.py:build_pigskin_rankings_sql`; read by ranking generator at `src/generate_pigskin_rankings.py:fetch_candidates`. | Safe for backend ranking evidence, not final ranking truth. |
| `analytics_pigskin_rankings` | output | Written by `src/generate_pigskin_rankings.py`; read by `src/player_profiles.py`. | Safe canonical ranking output. New rows include `model_run_id`; older rows may be NULL. |
| `analytics_pigskin_rankings_history` | output | Written by `src/materialize.py:build_pigskin_rankings_history_create_sql` and `src/generate_pigskin_rankings.py`. | Safe historical ranking output. New rows include `model_run_id`; older rows may be NULL. |
...
| `model_runs` | admin metadata | Created by `0002__create_model_runs.sql` and extended by `0003__model_run_config_foundation.sql`; helper writes and reads live in `src/model_runs.py:DEFAULT_MAX_VALUE_TABLES`, `src/model_runs.py:create_model_run`, `src/model_runs.py:mark_model_run_complete`, `src/model_runs.py:mark_model_run_failed`, and `src/model_runs.py:get_model_run`. | Safe lineage metadata. |
...
| `source_freshness_snapshots` | admin metadata | Created by `0003__model_run_config_foundation.sql`; helper writer is `src/model_runs.py:create_source_freshness_snapshot`, `src/model_runs.py:get_latest_model_run`. | Safe lineage metadata. |
| `active_league_rosters` | deprecated/unknown | Prompt reference only at `src/pigskin_context_tools.py`; no writer found in `app.py` or `src/`. | Not safe. |
| `historical_player_metrics` | deprecated/unknown | Prompt alias text at `src/pigskin_context_tools.py`; no writer found in `app.py` or `src/`. | Not safe. |
...
4. P0: Replace direct `weekly_metrics` reads in `src/sleeper_watch.py` and wire `src/trade_history.py` to `compat_trade_player_history` under a default-off flag.
